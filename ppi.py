...
def make_plots(df, true, feature1, feature2):
    # Line plots
    ...
    make_intervals(df[df["n"] == ns.min()], true, axs[0,:], feature1, feature2)

    make_histograms(df[df["n"] == ns.min()], axs[1,:], feature1, feature2)

    make_lineplots(df, axs[2,:], feature1, feature2)

    plt.tight_layout()
    os.makedirs("./ols-plots", exist_ok=True)
    plt.savefig("./ols-plots/results.pdf")

def make_lineplots(df, axs, feature1, feature2):
    plot_df = df[["coefficient", "estimator","width", "n"]].groupby(["coefficient", "estimator","n"], group_keys=False).mean()["width"].reset_index()
    lplt = sns.lineplot(data= plot_df[(plot_df["coefficient"] == feature1) & (plot_df["estimator"] != "imputed")], x="n", y="width", hue="estimator", ax=axs[0], hue_order=["prediction-powered", "classical"], legend = False)
    ...
    lplt = sns.lineplot(data=plot_df[(plot_df["coefficient"] == feature2) & (plot_df["estimator"] != "imputed")], x="n", y="width", hue="estimator", ax=axs[1], hue_order=["prediction-powered", "classical"], legend = False)
...
def make_intervals(df, true, axs, feature1, feature2):
    ci_imputed = df[(df["coefficient"] == feature1) & (df["estimator"] == "imputed")]
    ci_imputed = [ci_imputed["lb"].mean(), ci_imputed["ub"].mean()]
    ci_classical = df[(df["coefficient"] == feature1) & (df["estimator"] == "classical")]
    ci_classical = [ci_classical["lb"].mean(), ci_classical["ub"].mean()]
    ci = df[(df["coefficient"] == feature1) & (df["estimator"] == "prediction-powered")]
    ...
    axs[0].set_xlabel(feature1 + " coeff")
    axs[0].set_yticks([])
    ...
    # Sex coeff
    ci_imputed = df[(df["coefficient"] == feature2) & (df["estimator"] == "imputed")]
    ci_imputed = [ci_imputed["lb"].mean(), ci_imputed["ub"].mean()]
    ci_classical = df[(df["coefficient"] == feature2) & (df["estimator"] == "classical")]
    ci_classical = [ci_classical["lb"].mean(), ci_classical["ub"].mean()]
    ci = df[(df["coefficient"] == feature2) & (df["estimator"] == "prediction-powered")]
    ci = [ci["lb"].mean(), ci["ub"].mean()]
    ...
    axs[1].set_xlabel(feature2 + " coeff")
    axs[1].set_yticks([])
    axs[1].set_yticklabels([])
    axs[1].legend(bbox_to_anchor = (1.1,1), borderpad=1, fontsize=15) # bbox_to_anchor = (0.3,-3)
    axs[1].xaxis.set_tick_params()
...
def make_histograms(df, axs, feature1, feature2):
    # Width figure
    kde0 = sns.kdeplot(df[(df["coefficient"]==feature1) & (df["estimator"] != "imputed")], ax=axs[0], x="width", hue="estimator", hue_order=["prediction-powered", "classical"], fill=True, clip=(0,None), cut=0, legend = False)
    axs[0].set_ylabel("")
    ...
    kde1 = sns.kdeplot(df[(df["coefficient"]==feature2) & (df["estimator"] != "imputed")], ax=axs[1], x="width", hue="estimator", hue_order=["prediction-powered", "classical"], fill=True, clip=(0,None), cut=0, legend = False)
    axs[1].set_ylabel("")
    ...
    cvg_classical_age = (df[(df["estimator"]=="classical") & (df["coefficient"]==feature1)]["covered"]).astype(int).mean()
    cvg_classical_sex = (df[(df["estimator"]=="classical") & (df["coefficient"]==feature2)]["covered"]).astype(int).mean()
    cvg_predictionpowered_age = (df[(df["estimator"]=="prediction-powered") & (df["coefficient"]==feature1)]["covered"]).astype(int).mean()
    cvg_predictionpowered_sex = (df[(df["estimator"]=="prediction-powered") & (df["coefficient"]==feature2)]["covered"]).astype(int).mean()
